...
## 9. שכבת AI Tier-1 — אופציונלי (ממומש, כבוי כברירת מחדל)
- `ClaudeClient` (thin client על Http, מודל `claude-opus-4-8`, structured outputs) — מופעל רק כש-`AI_ENABLED=true` ומוגדר `ANTHROPIC_API_KEY`; אחרת כל השכבה no-op ולא שוברת כלום.
- `ClassifyTicketJob` — מסווג פנייה חדשה (עדיפות + קטגוריה), מעדכן את עדיפות הכרטיס ורושם הערה פנימית עם הסיווג.
- `DraftReplyJob` — מנסח **טיוטת תשובה** ושומר אותה כ**הערה פנימית** בלבד (author=ai). `SendTicketReplyJob` מדלג על הערות פנימיות — ולכן **שום דבר לא נשלח ללקוח ללא אישור אנושי**.
- **Human-in-the-loop:** פעולת "אישור טיוטת AI" ב-Filament פותחת את הטיוטה לעריכה; רק כשהסוכן לוחץ שליחה נוצרת הודעה יוצאת ומנותבת לערוץ. הסוכן שולט בכל פעולה יוצאת.
...
- **שלב 5 — AI Tier-1** ✅ אופציונלי (כבוי כברירת מחדל): סיווג + ניסוח טיוטות דרך Claude API, עם אישור אנושי לפני כל שליחה.
