...
- בנוסף, כל בוקר ב-09:00 נבדק אם בכלל הושלם גיבוי ב-36 השעות האחרונות
  (`BACKUP_STALE_AFTER_HOURS`). זה התופס את המקרים שלא משאירים שורה כושלת
  בכלל — worker שאינו רץ, מתזמן שלא הופעל מחדש אחרי דיפלוי.
...
- **התאוששות מאובדן מלא של השרת:** מתקינים מחדש, מריצים `php artisan migrate`,
  ...
  ביעד**. רשימת הגיבויים עצמה אינה נכללת בארכיון (שחזור שלה היה מוחק את הרשימה
