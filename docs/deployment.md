...
- **לפני שחזור: עצרו את ה-worker של התור** (`php artisan horizon:terminate` או עצירת
  ה-container של התור). חיובים והנפקת חשבוניות נעצרים מעצמם כל עוד שחזור רץ, אבל
  משימה שכבר באמצע קריאה לקארדקום/לינט אי אפשר להחזיר — הכסף יורד או המסמך מונפק,
  והשורה שרושמת אותם מוחלפת באותו רגע.
