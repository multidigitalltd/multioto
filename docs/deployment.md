...
- **נוהל השחזור — לעצור את ה-worker ואז להריץ את השחזור מהשורה:**
  ...
  **השורה האמצעית אינה קישוט.** `horizon:terminate` מבקש עצירה מסודרת ומחזיר
  שליטה מיד, בעוד המשימות שכבר רצות ממשיכות עד שיסתיימו. משימה שנמצאת באותו רגע
  בתוך קריאה לקארדקום או ללינט כבר עברה את השער — הכסף יורד או המסמך מונפק — ואם
  השחזור מתחיל באותו רגע, השורה שרושמת את זה מוחלפת. לכן ממתינים עד ש-Horizon
  מדווח `inactive` (בעצירת container: עד שהוא באמת יצא). הפקודה `backup:restore`
  גם בודקת בעצמה כמה משימות נותרו בתור ומבקשת אישור אם יש.
