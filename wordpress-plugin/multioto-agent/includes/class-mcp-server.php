...
class Multioto_Agent_Mcp_Server
{
    ...
    private function toolDefinitions(): array
    {
        ...
        if (Multioto_Agent_Elementor::active()) {
            $tools[] = ['name' => 'wp_elementor_texts_get', 'description' => 'כל הטקסטים הניתנים לעריכה בעמוד שבנוי באלמנטור, לפי id: לכל טקסט — מזהה הרכיב (widget_id), סוג הרכיב, שם השדה והטקסט הנוכחי. עמוד אלמנטור אינו שומר את התוכן שלו ב-content הרגיל, ולכן זו הדרך היחידה לראות ולערוך את מה שמופיע בו בפועל.', 'annotations' => $read, 'inputSchema' => ['type' => 'object', 'properties' => ['id' => ['type' => 'integer']], 'required' => ['id']]];
            $tools[] = ['name' => 'wp_elementor_text_update', 'description' => 'החלפת טקסט אחד בעמוד אלמנטור לפי widget_id (מתוך wp_elementor_texts_get). אופציונלי setting — כשלרכיב יש כמה שדות טקסט. מחזיר את הטקסט הקודם לצורך ביטול, ומרענן את ה-CSS של העמוד. שינוי מבנה, עיצוב או סדר רכיבים אינו נתמך בכוונה.', 'annotations' => $change, 'inputSchema' => ['type' => 'object', 'properties' => ['id' => ['type' => 'integer'], 'widget_id' => ['type' => 'string'], 'text' => ['type' => 'string'], 'setting' => ['type' => 'string']], 'required' => ['id', 'widget_id', 'text']]];
        }
