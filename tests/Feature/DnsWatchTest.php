...
class DnsWatchTest extends TestCase
{
    ...
    private function fakeDns(?array $a, ?array $mx, ?array $ns): void
    {
        $lookup = Mockery::mock(DnsLookup::class.'[query]')
            ->shouldAllowMockingProtectedMethods()
            ->makePartial();
        $lookup->shouldReceive('query')->with(Mockery::any(), DNS_A)
            ->andReturn($a === null ? null : array_map(fn (string $ip): array => ['ip' => $ip], $a));
        $lookup->shouldReceive('query')->with(Mockery::any(), DNS_MX)
            ->andReturn($mx === null ? null : array_map(fn (string $h): array => ['target' => $h], $mx));
        $lookup->shouldReceive('query')->with(Mockery::any(), DNS_NS)
    ...
    {
        $this->fakeDns(['1.2.3.4'], ['Mail.Example.com.'], ['ns1.example.com', 'ns2.example.com']);
        $this->quietTeam();
        ...
        $this->assertSame(['mail.example.com'], $snap['records']['mx']);
    ...
    public function test_a_changed_a_record_alerts_the_team_with_the_diff(): void
    {
        $site = Site::factory()->create([
            ...
            'dns_snapshot' => [
                ...
                'records' => ['a' => ['1.2.3.4'], 'mx' => ['mail.example.com'], 'ns' => ['ns1.example.com']],
                'changed_at' => null,
            ],
        ]);

        $this->fakeDns(['9.9.9.9'], ['mail.example.com'], ['ns1.example.com']);
    ...
    public function test_unchanged_records_do_not_alert_or_stamp_a_change(): void
    {
        ...
        $this->fakeDns(['1.2.3.4'], ['MAIL.example.com.'], ['NS1.EXAMPLE.COM']);
        $this->quietTeam();
