...
<head>
    ...
    <title>הכרטיס לא אושר — מולטי דיגיטל</title>
...
<body>
    <main>
        <div class="icon">💳</div>
        <h1>הכרטיס לא אושר</h1>
        @if (filled($reason ?? null))
            {{-- Cardcom writes these for the card holder, and they say the one
                 thing that helps. Escaped like any other external text. --}}
            <div class="notice"><div class="notice-body">{{ $reason }}</div></div>
        @endif
        <p>לא הצלחנו לשמור את הכרטיס, ולכן ההרשמה טרם הושלמה. אפשר לנסות שוב עם אותו כרטיס או עם כרטיס אחר — הפרטים שמילאתם שמורים.</p>
