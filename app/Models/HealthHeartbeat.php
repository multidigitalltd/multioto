...
class HealthHeartbeat extends Model
{
    ...
    /**
     * The last time a job on the ORDINARY queue actually finished.
     *
     * Stamped by the worker itself, after every job it completes, and it is
     * what separates a worker grinding through a long batch — where the beat
     * behind those jobs waits its turn for as long as the batch takes — from a
     * worker that is simply gone. Queue DEPTH cannot tell them apart: work
     * arriving as fast as it is finished leaves the depth flat while jobs
     * complete one after another. Only a count of finished work is monotone.
     */
